...
from .symbols import all_phonemes, _punctuations
phonemizer = Phonemizer.from_checkpoint(str(cached_path("hf://mrfakename/deep-phonemizer/en_us_cmudict_ipa.pt")))
...
class Phonemizer:
    ...
    def __call__(self, text: Union[str, list], with_stress=None, njobs=8, language=None) -> Union[str, list]:
        language = language or self.language
        njobs = njobs or self.njobs
        with_stress = with_stress or self.with_stress
        # phonemizer does not like hyphens.
        text = self._preprocess(text)
        phonemes = phonemizer(text, lang='en_us', batch_size=njobs)
